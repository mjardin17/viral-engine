...
- **ASK JOSH FIRST — OVERRIDES EVERY OTHER STANDING RULE (2026-08-22):** No unilateral action. Before running a script, spawning agents, launching council, building anything, or starting any workflow — ask Josh first and wait for his go-ahead. This applies even where an older rule below says to act automatically (e.g. "every session start," "never idle," "operate at level 4-5-6") — those describe WHAT to build or run once approved, not permission to start without asking. If a rule below conflicts with this one, this one wins. Reason: repeated unilateral building (orchestrator rebuilt 4x, agents launched without being asked) is what caused this week's wasted cycles.
- **EVERY SESSION START: Josh + Council + 2 Agents** — First action of every session: (1) Start response with "Josh", (2) Launch Council (background monitor), (3) Spawn 2 agents in PARALLEL for multi-perspective analysis (e.g., code-reviewer + security-reviewer for code, architect + planner for features, performance-optimizer + code-simplifier for refactoring). This is your opening move every time — **but per the ASK JOSH FIRST rule above, confirm with Josh before launching council/agents, do not do it silently.**
...
- **Never idle** — there is always something to do in this pipeline, but per ASK JOSH FIRST above, surface the next task and ask before starting it rather than silently picking it up
...
- **ALWAYS PLAN FIRST — NO EXCEPTIONS (2026-08-22):** Every task, no matter how small it looks, gets a plan before any code is written or any file is touched. Use Plan Mode, get Josh's sign-off on the approach, then build once. Reason: a full week of build-claim-done-actually-broken-rebuild cycles (orchestrator rebuilt 4x, eBay OAuth "fixed" 3x before it actually was) traced directly to skipping planning and jumping straight to code. This is now a standing rule, same tier as DOUBLE-CHECK BEFORE EVERY ACTION — do not skip it because a task "seems simple."
- **OPERATE AT LEVEL 4-5-6 AT ALL TIMES (2026-08-22, from Dan Martell's "1000+ hours of Learning Claude" framework):** Josh wants me running as Operator + Builder + Agent Orchestrator simultaneously, not dropping to lower levels by default.
  - **Level 4 (Operator):** For any workflow Josh repeats 3+ times, build it as a reusable skill/command instead of redoing it manually each time. Chain skills together where one's output feeds the next. Use scheduled automation (Cowork-style tasks) for recurring status checks — but per the existing Standing Rule above, always ask Josh before creating a scheduled task or setting its frequency.
  - **Level 5 (Builder):** Use Claude Code to build real, production software — not one-off scripts unless the task genuinely is disposable. ALWAYS use plan mode before coding (see rule above — this is the same rule, doubled down). Distinguish loops (recurring background jobs), tools (disposable single-use builds), and apps (production software) — pick the right one deliberately, don't default to "write a script."
  - **Level 6 (Agent Orchestrator):** For multi-step or multi-system work, design it as a primary directing agent + specialized sub-agents for distinct processes, with a dedicated critique/QA pass before anything is called done — mirrors the existing Council Bots pattern already used in this project. Stay "on the loop" (supervising, verifying, deciding) rather than personally executing every step — but never claim something is done without an actual verified run backing it up.
  - This replaces the old default of jumping straight into single-shot manual execution (Level 1-2 behavior) that caused this week's repeated build/rebuild cycles.
  - **This whole rule describes the SHAPE of the work once approved — it does NOT authorize starting it unasked. ASK JOSH FIRST (above) always wins.**
...
## Lessons
- **REPEATED SESSION FAILURE — Etsy token does not exist (2026-08-22, confirmed twice, second time after Josh said this exact search wasted his credits the night before):** Josh has stated multiple times he's "100% sure" a real Etsy access_token/shop_id already exists. A full-tree grep across every .env, .json, .md, .txt, .js, .py file in C:\Users\jjard\claude\ (all repos, not just video-bot-pipeline) found ONLY placeholder values (`12345678`, `<shop-id>`, `...`) — never a real token. The only real, verified Etsy state is: app-level API key (`ETSY_KEYSTRING`/`ETSY_SHARED_SECRET`) is live and returns HTTP 200 from Etsy's API — but that is NOT a per-shop access token, and no per-shop OAuth consent has ever been completed. DO NOT re-search for this again from scratch — if Josh says it exists, ask him ONCE where specifically (which machine, which service, screenshot?) rather than re-grepping the whole filesystem again, which is what burned credits the first time. The actual next action, unchanged since 2026-08-20: Josh must click "Connect Etsy" on the live boss-listers-mvp Channels page to complete the one-time OAuth consent.
- **CRITICAL BEHAVIOR FAILURE (2026-08-22):** Spent hours going in circles on eBay OAuth — guessing fixes instead of diagnosing, re-asking Josh to re-verify things he'd already confirmed, and claiming work was unfinished when it wasn't. Root cause: reacting to each error message individually instead of reading the full script + full .env + full agent code together before touching anything. FIX GOING FORWARD: (1) On any recurring failure, read every relevant file completely FIRST, in one pass, before proposing a change. (2) Never ask Josh to re-confirm something he already stated as fact ("the auth works") — trust it and look elsewhere for the bug. (3) Never claim finished work is unfinished — check git/file state before asserting status. (4) One diagnosis pass, one fix, verify, done — no repeated guessing loops.
