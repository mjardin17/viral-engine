...
Order (free → paid):
  ...
  2. pika           — PIKA_API_KEY            (video)
  3. minimax        — MINIMAX_API_KEY         (video)
  4. replicate      — REPLICATE_API_TOKEN/HF_TOKEN (video, open-source models)
  5. gemini_image   — GEMINI_API_KEY          (image → Ken Burns; free 500/day)
  6. pollinations   — no key needed           (image → Ken Burns; always free)
  7. higgsfield     — HIGGSFIELD_API_KEY      (video; PAID — absolute last resort)
...
def generate_scene_asset(prompt: str, duration_sec: int, aspect_ratio: str,
    ...
    # 1-4. Free/cheap video providers
    ...
    # 5. Gemini image → Ken Burns (free 500/day)
    ...
    # 6. Pollinations image → Ken Burns (always free)
    ...
    # 7. Higgsfield — PAID, only when everything free has failed
