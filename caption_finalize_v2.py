#!/usr/bin/env python3
"""
caption_finalize_v2.py — retrofit captions onto an already-rendered GG
episode and reassemble the final.mp4, WITHOUT re-fetching images or
re-doing TTS.

Usage:
    python3 caption_finalize_v2.py --episode GG_EP001 --music music/battle_epic.mp3 \
...
def run(episode_id: str, music: str, prompts_file: str) -> None:
    ...
    print(f"{episode_id}: {len(scenes)} scenes, captions source = {ep_path.name}", flush=True)
    ...
    for idx, scene in enumerate(scenes):
        ...
        if not clip_path.exists():
            print(f"  [{num:02d}] MISSING {clip_path.name} — skipping", flush=True)
            continue
    ...
    print(f"\nConcatenating {len(captioned_files)} captioned clips…", flush=True)
    ...
    print(f"\nDONE — {final_out} ({final_dur/60:.1f} min, {final_out.stat().st_size/1e6:.0f} MB)", flush=True)
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    run(args.episode.upper(), args.music, args.prompts_file)
